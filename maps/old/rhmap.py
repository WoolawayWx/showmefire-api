...
# Collect relative humidity data points from ALL stations
points = []
for s in stations:
...
if points:
    # Remove values that seem wrong (e.g., RH < 5 or RH > 100)
    ...
    if not filtered_points:
        ...
        # Interpolate relative humidity onto the grid
        ...
        # Plot the interpolated grid for relative humidity
        cs = ax.contourf(
...
# Plot stations only if network is AWOS, RAWS, or MOCOMAGNET
filtered_stations = [s for s in stations if s.get('network') in ('AWOS', 'RAWS', 'MOCOMAGNET')]
if filtered_stations:
    ...
    # Plot dots only for stations with RH value
    
    # Add RH labels at each site, but avoid overlap by skipping labels too close to previous ones
    label_positions = []
    min_dist = 0.12  # degrees, adjust as needed
    for s in filtered_stations:
        rh = s.get('observations', {}).get('relative_humidity', {}).get('value')
        if rh is not None:
            pos = np.array([s['longitude'], s['latitude']])
            # Only plot label if not too close to previous labels
            if all(np.linalg.norm(pos - np.array(lp)) > min_dist for lp in label_positions):
                ...
                # Add the RH label above the marker
                ax.text(
...
fig.text(
    ...
    "AWOS, RAWS, CWOP, Missouri Mesonet\n\n"
...
fig.savefig('images/rh.png', dpi=mapdpi, bbox_inches=None, pad_inches=0)


print(f"RH% Map updated at {pd.Timestamp.now(tz='America/Chicago').strftime('%Y-%m-%d %H:%M CT')}")

# filepath: /Users/cade/Development/Show Me Fire/api-server/showmefire-api/maps/rhmap-fil.py



# Define contour levels: every 1% from 0-20, then every 5% from 25-100
contour_levels = list(range(0, 21, 1)) + list(range(25, 101, 5))
...
if 'grid_values' in locals() and grid_values is not None:
    ...
    with open("gis/rh_fil_contours.geojson", "w") as f:
