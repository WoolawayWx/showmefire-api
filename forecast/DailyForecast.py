...
# Map house style lives in forecast/map_style.py (shared with the ensemble
# fire danger graphics); re-exported here so existing imports keep working.
from forecast.map_style import add_boundaries, add_title_and_branding, create_base_map  # noqa: E402
    ...
    for i, time_step in enumerate(ds_full.step):
        ...
        if has_precip:
            try:
                ...
                duration_values = decoded_precip.interval_hours.sel(step=time_step).values
                precip_interval_hours = float(np.nanmedian(duration_values))
...
def generate_complete_forecast():
    ...
    # Ensemble fire danger hand-off: persist this run's own hourly FM + grid
    # so services/ensemble_fire_danger can anchor its members on the same
    # operational fuel moisture. Best-effort; write() never raises.
    try:
        from services.ensemble_fire_danger import forecast_state as _ens_forecast_state
        _ens_forecast_state.write(pd.Timestamp(RUN_DATE), lat, lon, hourly_fm, swe_grid, SNOW_THRESHOLD_IN)
    except Exception as _ens_state_exc:
        logger.warning("Ensemble forecast-state hand-off failed (non-fatal): %s", _ens_state_exc)
