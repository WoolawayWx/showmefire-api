"""Covers Phase 2's fire_weather_ml pilot migration (second family, after
risk_fusion_glm): install_bundle() dual-writes into the unified registry,
and load_bundle() can resolve from the registry's `stable` channel. Uses
tests/test_fire_weather_ml_shadow.py's own _write_bundle() fixture shape."""
import json
import tempfile
import unittest
import zipfile
...
from tests.test_fire_weather_ml_shadow import _write_bundle
    ...
    def test_install_bundle_registers_a_beta_candidate(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with registry_patch, bundles_patch:
                ...
                result = shadow_bundles.install_bundle("fire_weather_ml", archive_path, uploaded_by="tester")

                self.assertIsNotNone(result["registry_version"])
                entry = versioning.get_model_entry("fire_weather_ml")
                beta = entry["beta"]
                ...
                self.assertEqual(beta["metadata"]["model_family"], "xgboost_regressor")
                self.assertIn("model", beta["assets"])
                self.assertIn("risk_calibration", beta["assets"])
...
class LoadBundleRegistryFallbackTests(unittest.TestCase):
    ...
    def test_falls_back_to_registry_stable_when_env_and_directory_are_unset(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self._isolated(root), patch.dict("os.environ", {fwms.BUNDLE_ENV: ""}, clear=False):
                assets = {name: str(bundle_dir / filename) for name, filename in fwms.BUNDLE_ASSET_FILENAMES.items()}
                versioning.register_trained_model(
                    "fire_weather_ml", channel="beta", assets=assets,
                    metadata={"advisory_only": True, "model_family": "xgboost_regressor",
                             "feature_module_sha256": "x", "label_module_sha256": "y", "label_column": "z",
                             "training_row_count": 50, "split_manifest_sha256": "w",
                             "shadow_bundle_version": "0.0.1-beta.7"},
                )
                versioning.promote("fire_weather_ml")

                bundle = fwms.load_bundle(directory=None)
    ...
    def test_explicit_directory_still_wins_over_registry(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self._isolated(root), patch.dict("os.environ", {fwms.BUNDLE_ENV: ""}, clear=False):
                assets = {name: str(registry_bundle_dir / filename) for name, filename in fwms.BUNDLE_ASSET_FILENAMES.items()}
                versioning.register_trained_model(
                    "fire_weather_ml", channel="beta", assets=assets,
                    metadata={"advisory_only": True, "model_family": "xgboost_regressor",
                             "feature_module_sha256": "x", "label_module_sha256": "y", "label_column": "z",
                ...
                bundle = fwms.load_bundle(directory=override_dir)
