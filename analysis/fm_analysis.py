...
def main(preferred_date=None, out_png='analysis/images/fm_analysis.png'):
    ...
    if points:
        ...
        cax = fig.add_axes([0.20, 0.08, 0.02, 0.6])
    ...
    try:
        ...
        ax.set_title(f"Observed Minimum Fuel Moisture — {title_date}", fontsize=18, fontweight='bold', pad=12)
    except Exception:
        pass
    fig.savefig(out_png, dpi=mapdpi, bbox_inches='tight', pad_inches=0.1)
    plt.close(fig)
    runtime_sec = time.time() - start_time
    print(f"Fuel Moisture Filtered Map updated at {pd.Timestamp.now(tz='America/Chicago').strftime('%Y-%m-%d %H:%M CT')}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out', help='Output PNG path', default='analysis/images/fm_analysis.png')
