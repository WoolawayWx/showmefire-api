...
def _collect_and_diagnose(preferred_date=None):
    ...
    for s in stations[:80]:
        ...
        if isinstance(obs, dict):
            for k, val in obs.items():
                if 'relative_humidity' in k.lower():
        ...
        if rh_val is None:
            for k, val in s.items():
                if isinstance(k, str) and 'relative_humidity' in k.lower():
                    if val is None:
                        continue
                    if isinstance(val, dict):
                        arr = val.get('value') or val.get('values') or None
    ...
    print(f"Loaded stations={total_stations}, filtered={total_filtered}, with_rh={with_rh}, with_latlon={with_latlon}")
    os.makedirs('logs', exist_ok=True)
    try:
        with open('logs/endofday_sample.json', 'w') as fh:
            json.dump({'sample': sample, 'rh_missing_examples': rh_missing_examples[:80]}, fh, indent=2)
    except Exception:
        pass
...
def main(preferred_date=None, out_png='analysis/images/mo-rh.png'):
    ...
    for s in filtered_stations:
        ...
        if rh is not None:
            try:
                ...
                continue
            capped_rh = min(float(rh), 100.0)
            points.append((lon, lat, capped_rh))
    ...
    if points:
        ...
        plt.colorbar(cs, cax=cax, label='Relative Humidity (%)', ticks=np.arange(0, 102, 5))
    ...
    try:
        ...
        ax.set_title(f"Observed Minimum Relative Humidity — {title_date}", fontsize=18, fontweight='bold', pad=12)
    except Exception:
        pass
    fig.savefig(out_png, dpi=mapdpi, bbox_inches='tight', pad_inches=0.1)
    plt.close(fig)
    runtime_sec = time.time() - start_time
    print(f"RH% Filtered Map updated at {pd.Timestamp.now(tz='America/Chicago').strftime('%Y-%m-%d %H:%M CT')}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out', help='Output PNG path', default='analysis/images/rh_analysis.png')
